...
function doPostSearch(q) {
  ...
  if (!q) {
    countEl.innerHTML = '<span>输入关键词开始搜索</span><span class="post-search-shortcuts">Enter: 下一个 | Esc: 关闭</span>';
  ...
  if (postSearchMatches.length === 0) {
    countEl.innerHTML = '<span>未找到匹配内容</span><span class="post-search-shortcuts">Enter: 下一个 | Esc: 关闭</span>';
  ...
  countEl.innerHTML = '<span>找到 ' + postSearchMatches.length + ' 处匹配</span><span class="post-search-shortcuts">Enter: 下一个 | Esc: 关闭</span>';
...
function updateNavState() {
  ...
  badge.textContent = (postSearchActiveIndex + 1) + '/' + total;
}
